...
One /goal/ of this library is to funnel log messages coming from
different libraries that talk to =slf4j= into =active-logger=.
...
any direct or transitive dependencies on =org.slf4j/slf4j-log4j12=.
...
before any configuration for =active-clojure= can be applied.
