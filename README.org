...
** Breaking changes in version 0.17.0:

- The api of ~active.clojure.logger.metric-accumulator~ has been
  greatly reduced. Much was moved and marked as 'internal' now.

** Breaking changes in version 0.15.0:

- Prometheus metrics do no longer render the timestamp of their last sample.
  Use events if you need data with timestamps.
...
be fixed.
** Jetty
[[https://www.eclipse.org/jetty/][Jetty]] before version 12 is one example of a bad player.  It will, as soon as any of it's
